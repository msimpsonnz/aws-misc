...
import { EventBus, Rule, EventField, IRuleTarget, RuleTargetInput } from '@aws-cdk/aws-events';
...
export class EbTransformStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    const fnGetData = new lambda.NodejsFunction(
      this,
      'fnGetData',
      {
        ...
        environment: {
          ...
          DB_PASSWORD: database.secret?.secretValueFromJson('password').toString() || 'error getting user secret',
    ...
    database.connections.addSecurityGroup(fnGetData.connections.securityGroups[0])

    //Create a queue for the events
    const queue = new sqs.Queue(this, 'queue')

    //Create rule for new SMS Message Received
    const processEvents = new Rule(
      this,
      "processEvents",
      {
        eventBus: eventBus,
    ...
    const logger = new Function(
      this,
      "s3-event-test-lambda",
      {
        code: Code.fromInline('exports.handler = async function(event) { console.log(JSON.stringify(event)); }'),
