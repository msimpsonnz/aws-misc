#####################
#
# Adapted for AWS Lambda from https://github.com/AndrewFarley/farley-aws-missing-tools/blob/master/aws-autoscaling-rollout/aws-autoscaling-rollout.py
#
#####################
import os
import boto3
...
def handler(event, context):
    asg = os.environ['autoscaler']
    autoscaler = get_autoscaling_group(asg)
    ...
    if autoscaler_old_max_size == autoscaler_old_desired_capacity:
        ...
        if update_auto_scaling_group_max_size(asg, (autoscaler_old_max_size + 1) ) is True:
    ...
    suspend_processes( asg, suspend_new_processes )
    ...
    if len(autoscaler['LoadBalancerNames']) > 0:
        ...
        for name in autoscaler['LoadBalancerNames']:
            ...
            wait_for_complete_loadbalancer_autoscaler_attachment( name, asg )
    ...
    if len(autoscaler['TargetGroupARNs']) > 0:
        ...
        for name in autoscaler['TargetGroupARNs']:
            print("Waiting for all instances to be healthy in " + name + "...")
            wait_for_complete_targetgroup_autoscaler_attachment( name, asg )
    ...
    set_desired_capacity( asg, autoscaler['DesiredCapacity'] + 1 )
    ...
    for i, instance in enumerate(instances_to_kill):
        ...
        # Wait to have healthy == desired instances on the autoscaler
        print("Ensuring that we have the right number of instances on the autoscaler")
        wait_for_autoscaler_to_have_healthy_desired_instances( asg )
        ...
        while True:
            ...
            new_current_instance_list = get_autoscaler_healthy_instances(asg)
        ...
        time.sleep(2)
        current_instance_list = get_autoscaler_healthy_instances(asg)
        ...
        if (i + 1) == len(instances_to_kill):
            terminate_instance_in_auto_scaling_group( instance['InstanceId'], asg, True )
            downscaled = True
        # Otherwise, simply kill this server and wait for it to be replaced, keeping the desired capacity
        else:
            terminate_instance_in_auto_scaling_group( instance['InstanceId'], asg )
    ...
    if downscaled == False:
        ...
        set_desired_capacity( asg, autoscaler_old_desired_capacity )
    ...
    resume_processes(asg, suspend_new_processes)
    ...
    if autoscaler_old_max_size == autoscaler_old_desired_capacity:
        ...
        if update_auto_scaling_group_max_size(asg, autoscaler_old_max_size ) is True:
